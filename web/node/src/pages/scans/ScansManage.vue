<template>
  <div :class="$style.main">
    ...
    <div :class="$style.sidebar">
      <router-link :class="getFilterClass()" :to="{ name: 'scans_manage' }">All</router-link>
      <router-link :class="getFilterClass(f)" v-for="f of filterOptions"
                   :to="{ name: 'scans_manage', query: { char: f } }" :key="f">{{ f }}
      </router-link>
    </div>
...
export default {
  ...
  data() {
    return {
      warningMsg: 'This action will permenently delete the source file and all attachments.'
    };
  },
  computed: {
    scans() {
